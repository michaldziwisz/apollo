...
class TextTests(unittest.TestCase):
	...
	def test_encode_text_fixes_isolated_digits_6_and_9(self) -> None:
		self.assertEqual(
			encode_text("6", expand_numbers=False, apply_polish_digit_fixes=True),
			encode_text("sześć", expand_numbers=False),
		)
		self.assertEqual(
			encode_text("9", expand_numbers=False, apply_polish_digit_fixes=True),
			encode_text("dziewięć", expand_numbers=False),
		)

	def test_encode_text_leaves_isolated_digits_when_polish_fix_is_disabled(self) -> None:
		self.assertEqual(encode_text("6", expand_numbers=False), b"6")
		self.assertEqual(encode_text("9", expand_numbers=False), b"9")

	def test_encode_text_does_not_fix_non_isolated_digits(self) -> None:
		self.assertEqual(encode_text("66", expand_numbers=False, apply_polish_digit_fixes=True), b"66")
		self.assertIn(b"6", encode_text("Za 6 dni", expand_numbers=False, apply_polish_digit_fixes=True))
