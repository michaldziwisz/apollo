...
# Apollo formant adjustment commands use a 1-byte hexadecimal delta (00..FF) with +/-.
# Clamp to the full documented range internally, but keep the UI range configurable so opening the
# settings dialog stays responsive (10 parameters * hundreds of drop-down items can be slow).
_FORMANT_DELTA_HARD_MIN = -255
_FORMANT_DELTA_HARD_MAX = 255
...
def _encodeText(text: str, *, expandNumbers: bool = True) -> bytes:
	if expandNumbers and any("0" <= ch <= "9" for ch in text):
...
class SynthDriver(BaseSynthDriver):
	...
	def __init__(self):
		...
		# the "@I?" / "@I+" form.
		self._indexQueryCommand = b"@I?"
		self._indexEnableCommand = b"@I+ "
		self._indexMarkCommand = b"@I+ "
	...
	def _getFormantCommandsFromDeltas(self, deltas: Sequence[int]) -> list[str]:
		formantCommands: list[str] = []
		for d, delta in enumerate(deltas):
			if not delta:
				continue
			deltaInt = max(_FORMANT_DELTA_HARD_MIN, min(_FORMANT_DELTA_HARD_MAX, int(delta)))
			sign = "+" if deltaInt > 0 else "-"
			hh = min(0xFF, abs(deltaInt))
			formantCommands.append(f"@u{d}{hh:02X}{sign} ")
		return formantCommands
	...
	def _getFormantDiffCommands(self, desired: Sequence[int], applied: Sequence[int]) -> list[str]:
		...
		for index, delta in enumerate(desired):
			...
			if diff:
				commands.extend(self._getFormantAdjustCommands(index, diff))
		return commands

	def _getFormantAdjustCommands(self, index: int, diff: int) -> list[str]:
		if not diff:
			return []
		sign = "+" if diff > 0 else "-"
		...
		def flushText() -> None:
			...
			if text:
				...
				outputParts.append(
					_encodeText(
						text,
						expandNumbers=self._expandNumbers and not (synthSpellMode or charModeActive),
					),
				)
